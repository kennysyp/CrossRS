...
class Dataset_epoch_crop_mt(Data.Dataset):
    def __init__(self, names, norm=False):
        'Initialization'
        super(Dataset_epoch_crop_mt, self).__init__()
        self.names = names
        self.norm = norm
    ...
    def __getitem__(self, step):
        # Select sample
        img = load_4D_with_crop(self.names[step]['img'], cropx=160, cropy=192, cropz=160)
        seg = load_4D_with_crop(self.names[step]['seg'], cropx=160, cropy=192, cropz=160)

        if self.norm:
            return {"img": torch.from_numpy(imgnorm(img)).float(), "seg": torch.from_numpy(seg).float()}
        else:
            return {"img": torch.from_numpy(img).float(), "seg": torch.from_numpy(seg).float()}
...
class Dataset_epoch_crop(Data.Dataset):
    ...
    def __getitem__(self, step):
        # Select sample
        img_A = load_4D_with_crop(self.index_pair[step][0], cropx=160, cropy=192, cropz=160)
        img_B = load_4D_with_crop(self.index_pair[step][1], cropx=160, cropy=192, cropz=160)
...
class Predict_dataset_crop(Data.Dataset):
    ...
    def __getitem__(self, step):
        # Select sample
        img_A = load_4D_with_crop(self.index_pair[step][0]['img'], cropx=160, cropy=192, cropz=160)
        img_B = load_4D_with_crop(self.index_pair[step][1]['img'], cropx=160, cropy=192, cropz=160)
        label_A = load_4D_with_crop(self.index_pair[step][0]['seg'], cropx=160, cropy=192, cropz=160)
        label_B = load_4D_with_crop(self.index_pair[step][1]['seg'], cropx=160, cropy=192, cropz=160)
...
class Dataset_pairs_crop(Data.Dataset):
    ...
    def __getitem__(self, step):
        # Select sample
        img_A = load_4D_with_crop(self.data_pairs[step]['img1'], cropx=160, cropy=192, cropz=160)
        img_B = load_4D_with_crop(self.data_pairs[step]['img2'], cropx=160, cropy=192, cropz=160)

        if "seg1" in self.data_pairs[step].keys():
            seg_A = load_4D_with_crop(self.data_pairs[step]['seg1'], cropx=160, cropy=192, cropz=160)
            seg_A = torch.from_numpy(seg_A)
        else:
            seg_A = None
        if "seg2" in self.data_pairs[step].keys():
            seg_B = load_4D_with_crop(self.data_pairs[step]['seg2'], cropx=160, cropy=192, cropz=160)

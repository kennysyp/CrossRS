...
if __name__ == "__main__":
    ...
    model = U_Network(3, nf_enc, nf_dec).cuda()

    dummy_input = torch.rand(1, 2, 192, 192, 128).cuda()

    example = model(dummy_input)
    print(example.shape)

    flops = FlopCountAnalysis(model,  (dummy_input, ))
    params = sum([param.nelement() for param in model.parameters()])
    print(f"FLOPs: {(flops.total() / 1e9):.2f} G, Params: {(params / 1e6):.2f} M")
